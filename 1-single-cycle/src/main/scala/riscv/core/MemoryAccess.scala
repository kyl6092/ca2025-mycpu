  ...
  // - SH to address 0x1002 (index=2): data[15:0] → bytes 2-3, strobes[2:3]=1
    ...
    switch(io.funct3) {
      is(InstructionsTypeS.sb) {
        ...
        writeData := data(7,0) << (mem_address_index << 3) // Byte address times 8
      }
